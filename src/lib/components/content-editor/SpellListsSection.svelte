...
<script lang="ts">
  ...
  function addSpellListClass(classId: string): void {
    (ctx.feature as MagicFeature).spellLists = { ...(magic.spellLists ?? {}), [classId]: 0 };
  }
  ...
  function setSpellListLevel(classId: string, level: number): void {
    (ctx.feature as MagicFeature).spellLists = { ...(magic.spellLists ?? {}), [classId]: level };
  }
...
<details class="group/sl rounded-lg border border-border overflow-hidden">
  <summary class="flex items-center justify-between px-4 py-2 bg-surface-alt cursor-pointer
    ...
    {ui('editor.spell_lists.section_label', lang)}
  ...
  <div class="p-4 flex flex-col gap-3">
    <p class="text-[11px] text-text-muted">
      {ui('editor.spell_lists.section_hint', lang)}
    </p>

    {#if spellListEntries.length === 0}
      <p class="text-xs text-text-muted italic">{ui('editor.spell_lists.no_spell_lists', lang)}</p>
    {:else}
      <table class="w-full text-xs border-collapse">
        <thead>
          <tr class="border-b border-border">
            <th class="text-left px-2 py-1 font-semibold text-text-muted">{ui('editor.spell_lists.col_class', lang)}</th>
            <th class="text-center px-2 py-1 font-semibold text-text-muted w-20">{ui('editor.spell_lists.col_level', lang)}</th>
            <th class="w-8"></th>
          </tr>
        </thead>
        <tbody>
            ...
            <tr class="border-b border-border/50 hover:bg-surface-alt/50">
              ...
              <td class="px-2 py-1 text-center">
                <input type="number" class="input text-xs text-center w-14 py-0.5" min="0" max="9"
                       ...
                       aria-label={ui('editor.spell_lists.spell_level_aria', lang).replace('{id}', classId)}/>
              </td>
              <td class="px-2 py-1 text-center">
                <button type="button" class="btn-ghost btn-icon h-6 w-6 p-0 text-text-muted hover:text-danger"
                        ...
                        aria-label={ui('editor.spell_lists.remove_aria', lang).replace('{id}', classId)}>
    ...
    <button type="button" class="btn-primary text-xs py-1 px-3 h-auto w-fit"
      ...
      {ui('editor.spell_lists.add_class_btn', lang)}
    </button>
  </div>
</details>
