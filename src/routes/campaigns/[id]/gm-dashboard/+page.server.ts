/**
 * @file src/routes/campaigns/[id]/gm-dashboard/+page.server.ts
 * @description Server-side guard for the GM-only Entity Dashboard page.
 *
 * DEFENCE-IN-DEPTH:
 ...
 *     for session data).
 *
 * @see src/routes/campaigns/[id]/gm-dashboard/+page.svelte — client-side GM guard
...
export const load: PageServerLoad = ({ cookies, params }) => {
  ...
  if (!session) {
    redirect(303, `/login?redirect=/campaigns/${params.id}/gm-dashboard`);
  }
