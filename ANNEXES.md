...
> **Convention — UI chrome vs. game content translations:** Rule JSON files (under `static/rules/`) translate game *content* (feature labels, descriptions) using `LocalizedString` objects. They are separate from *UI chrome* translations (buttons, nav, error messages), which live in `static/locales/{code}.json`. Annex examples show only game-content translations. For the UI locale file format (including the mandatory `$meta.countryCode` field for country flag rendering), see ARCHITECTURE.md §11.6 and CONTENT_AUTHORING_GUIDE.md §23.
