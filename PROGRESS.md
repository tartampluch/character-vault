...
- [x] **14.6 Frontend Integration (StorageManager):** Refactor `StorageManager.ts` to use async `fetch()` calls to PHP API. Server-first character loading: every `/character/[id]` page load fetches `GET /api/characters/{id}` directly. No auto-save — characters are saved only via an explicit Save button. `ApiError` class surfaced to the UI with specific handling for 409 Conflict (stale save) and 429 (rate limit). `GET /api/characters/{id}` (show endpoint) added to PHP router. `updatedAt` concurrency check on `PUT /api/characters/{id}` — server returns 409 if client timestamp is older than DB row. localStorage retains only UI preferences (settings, language, active char ID); character data blobs and polling timestamps removed.
...
- [x] **21.1.4 HomebrewStore:** Create `src/lib/engine/HomebrewStore.svelte.ts`. A Svelte 5 `$state`-based store managing homebrew entities for the current campaign. Tracks `scope: 'campaign' | 'global'` and `filename: string` (used when `scope === 'global'`; validated as `[0-9a-z_-]+\.json`; default `"50_homebrew.json"`). Methods: `add(entity: Feature): void`, `update(id: ID, patch: Partial<Feature>): void`, `remove(id: ID): void`, `getById(id: ID): Feature | undefined`, `toJSON(): string`, `save(): Promise<void>` (explicit save — called by EntityForm's Save button). Exposes reactive `isDirty: boolean` and `isSaving: boolean` flags. `save()` throws on any server error so the UI can display the message.
