...
- [x] **Checkpoint #2 — UI Layer Conformance** (requires Phases 1–13): Run from `CHECKPOINTS.md`. Resolve ALL issues (CRITICAL, MAJOR, and MINOR) before proceeding. *(30th pass — resolved: guessGrantCategory() in FeatureModal.svelte had hardcoded D&D ID prefix strings ('feat_', 'spell_', 'item_', etc.) inline — moved to FEATURE_ID_CATEGORY_PREFIXES constant in constants.ts per zero-hardcoding rule ARCHITECTURE.md §6. svelte-check: 0 errors, 0 warnings.)*
...
- [x] **Checkpoint #6 — UI Excellence** (requires Phase 19): Run from `CHECKPOINTS.md`. Resolve ALL issues (CRITICAL, MAJOR, and MINOR) before proceeding. *(Re-run pass — resolved: 1 CRITICAL badge-blue class undefined in app.css (SkillsMatrix/LevelingJournalModal rendered unstyled); 3 MAJOR issues: btn-icon+btn-danger-outline excluded from 44px coarse touch rule, sidebar nav links excluded from 44px anchor rule (navLinkClass now carries explicit min-h-[44px]), tablet 768–1023px showed mobile overlay drawer instead of icon-only inline column (Sidebar.svelte now uses md: breakpoint classes + effectivelyCollapsed derived, AppShell top-bar/backdrop changed from lg:hidden to md:hidden); 4 MINOR issues: HorizontalScroll scrollToDot used behavior:smooth ignoring prefers-reduced-motion, LevelingJournalModal missing fullscreen prop, icon sizes size=18→20 and size=15→16 across multiple files, sidebar language select used focus: instead of focus-visible:. svelte-check: 0 errors, 0 warnings.)* *(3rd pass — resolved: 1 MINOR issue: character sheet tab buttons used min-h-[40px] utility which overrides the @layer base coarse-pointer 44px rule (Tailwind utilities take precedence over base layer), resulting in 40px tap targets on touch devices — changed to min-h-[44px] in src/routes/character/[id]/+page.svelte:360. All Tailwind migration, theme system, icon sizing, sidebar breakpoints, full-height layout, responsive design, and touch/accessibility criteria verified passing. svelte-check: 0 errors, 0 warnings.)* *(4th pass — resolved 6 MINOR issues: (1) HealthAndXP.svelte XP bar used literal oklch(72% 0.17 88) instead of var(--color-gold) already defined in app.css:138; (2) HP_STATUS_MAP used 7 hardcoded oklch literals — replaced with var(--color-red-900/-800/-600), var(--color-gold), var(--color-green-500), var(--theme-text-muted); (3) Encumbrance.svelte loadTier map used 5 hardcoded oklch literals — replaced with same CSS variable pattern; (4) SkillsMatrix.svelte and PointBuyModal.svelte over-budget progress bar used oklch(55% 0.20 28) — replaced with var(--color-red-600); (5) character sheet tab bar used inline style="scrollbar-width: none; scroll-snap-type: x mandatory;" — added .scrollbar-none component class to app.css and replaced with snap-x snap-mandatory scrollbar-none Tailwind/component classes; (6) CharacterCard.svelte, campaigns/+page.svelte, campaigns/[id]/+page.svelte campaign/character placeholder gradients used hardcoded dark oklch values not theme-aware in light mode — replaced with Tailwind bg-gradient-to-br from-accent-100/red-100 to-accent-50/red-50 dark:from-accent-950/red-950 dark:to-accent-900/red-900 classes. svelte-check: 0 errors, 0 warnings.) *(5th pass — resolved 2 MINOR issues: (1) CoreCombat.svelte CORE_STATS array used 5 hardcoded oklch() literals as inline style values (BAB/Initiative/Grapple/Fortification/ASF) — replaced with var(--color-stat-bab/initiative/grapple/fortification/asf) CSS custom properties added to app.css :root; (2) ArmorClass.svelte AC_PIPELINES array used 3 hardcoded oklch() literals (Normal/Touch/Flat-Footed) — replaced with var(--color-ac-normal/touch/flat). All 8 new CSS variables defined in app.css :root with D&D-contextual comments. svelte-check: 0 errors, 0 warnings.)*
...
- [x] **Checkpoint #7 — Leveling Progression** (requires Phase 20): Run from `CHECKPOINTS.md`. Resolve ALL issues (CRITICAL, MAJOR, and MINOR) before proceeding. *(Re-run pass — resolved 1 MINOR issue: `phase_multiclassXpPenaltyRisk` in GameEngine.svelte.ts did not exclude the character's favored class from the XP penalty check (PROGRESS.md Phase 20.2 requires "favored-class exemption"). Fix: added `favoredClass?: ID` field to `Character` in `src/lib/types/character.ts` with full SRD-accurate JSDoc, and updated `phase_multiclassXpPenaltyRisk` to filter out the favored class before computing the max and checking the 2+ level gap. All other sections — SkillsMatrix, LevelingJournalModal, i18n keys (EN + FR), multiclass SP budget tests, minimum rank enforcement tests, character build scenario (50 SP / +5 BAB / Fort +10 / Ref +7 / Will +10 / 75 HP / AC 15 / 5 feat slots / PP 3 / 3/2 Wizard spells, 100+ assertions) — verified fully conformant. svelte-check: 0 errors, 0 warnings. 1765/1765 tests pass.)*
...
_Goal: A focused iteration addressing player-facing visibility rules, the authentication flow, persistent character lore fields, and seed data quality._
