...
- [x] **11.2 Feats Tab Layout & Lists:** Create `src/lib/components/feats/FeatsTab.svelte`. Header: "Feats Available" / "Feats Left" counters. Granted Feats section (auto-granted by Race/Class — read-only, shows source tag). Selected Feats section (player-chosen, with Delete button to free slot).

- [x] **11.3 Feat Catalog Modal:** Create `src/lib/components/feats/FeatSelectionModal.svelte`. Fetches all `category: "feat"` features from `DataLoader`. Text search (title/description). Tag badges (e.g. "Fighter Bonus Feat", "Metamagic").
...
- [x] **12.1 Magic Resources & Limits:** In `GameEngine`, derive `Caster Level`, `Manifester Level` pipelines. Derive `Spell Slots` (per level 0–9) and `Power Points` max from class JSON formulas and key ability modifier.
...
- [x] **12.4 Special & Domain Abilities Panel:** Create `src/lib/components/magic/SpecialAbilities.svelte`. Filters active features for `class_feature` or `domain` category with `activation` type. Displays as cards tied to `ResourcePool`s. "Use" button calls `engine.spendItemPoolCharge()` or decrements resource pool.
...
- [x] **14.6 Frontend Integration (StorageManager):** Refactor `StorageManager.ts` to use async `fetch()` calls to PHP API. Debounced auto-save (2s). Graceful offline fallback to localStorage. Polling mechanism: check `sync-status` every 5–10s, selectively re-fetch changed data.
...
_These conversion tasks are tracked in detail in `D20SRD_CONVERSION.md` and represent completed JSON data authoring work. Code phases above are independent of these tasks._

- [x] **C-15a** — 4 psionic classes → `01_d20srd_psionics/00_d20srd_psionics_classes.json`
- [x] **C-15b** — 60 psionic class features → `01_d20srd_psionics/01_d20srd_psionics_class_features.json`
- [x] **C-15c** — 194 psionic powers A–M → `01_d20srd_psionics/02_d20srd_psionics_powers.json` (part 1)
- [x] **C-15d** — 93 psionic powers N–Z → `01_d20srd_psionics/02_d20srd_psionics_powers.json` (part 2, total 287 powers)
- [x] **C-15e** — 80 psionic feats → `01_d20srd_psionics/03_d20srd_psionics_feats.json`
- [x] **C-15f** — 6 psionic races → `01_d20srd_psionics/04_d20srd_psionics_races.json`
- [x] **C-15g** — 4 psionic skills + psionic Concentration additions → `00_d20srd_core/04_d20srd_core_skills_config.json`
- [x] **C-15h** — 9 psionic prestige classes → `01_d20srd_psionics/05_d20srd_psionics_prestige_classes.json`
- [x] **C-15i** — 97 psionic prestige class features + racial features → `01_d20srd_psionics/06_d20srd_psionics_prestige_class_features.json`
- [x] **C-15j** — 63 psionic items (cognizance crystals, dorjes, tattoos, psicrowns, universal items, artifacts) → `01_d20srd_psionics/07_d20srd_psionics_items.json`
- [x] **C-15k** — 8 psionic spells (arcane/divine interactions) → appended to `00_d20srd_core/05_d20srd_core_spells.json`
