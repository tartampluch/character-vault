...
- [x] **1.3e Feature model extensions — item mechanics & activation variants:** Extended `src/lib/types/feature.ts` with four additions that complete the SRD item and activation model. (a) **`ResourcePoolTemplate` interface** (exported): `poolId`, `label`, `maxPipelineId`, `defaultCurrent`, `resetCondition`, optional `rechargeAmount` — the static schema for per-instance item charges. Added `Feature.resourcePoolTemplates?: ResourcePoolTemplate[]` to the base `Feature` interface. (b) **`ActivationTier` interface** (exported): `label`, `targetPoolId`, `cost` (number or formula), `grantedModifiers` — one option in a variable-cost activation. Added `Feature.activation.tieredResourceCosts?: ActivationTier[]`. (c) **`activation.actionType` union extension**: added `"passive"` (always active, no button) and `"reaction"` (fires automatically), bringing the union to 10 values. (d) **`activation.triggerEvent?: string`** — the event string that fires a reaction feature (`"on_fall"`, `"on_spell_targeted"`, `"on_damage_taken"`, `"on_attack_received"`). Documented in `ARCHITECTURE.md` section 5.7 (pool template lifecycle) and section 5.5b (passive/reaction with standard trigger-event table). Chameleon Power ring's conditionNode on Hide modifier added (activation:free pattern). SRD rings updated: Ring of Feather Falling (`reaction`/`on_fall`), Ring of Counterspells (`reaction`/`on_spell_targeted`), skill rings (`passive`), Ring of the Ram tiered charges (3 tiers), Elemental Command rings per-day/per-week ability pools.
...
- [x] **1.4a `ActiveFeatureInstance` — instance-scoped item resource pools:** Extended `ActiveFeatureInstance` in `src/lib/types/character.ts` with optional `itemResourcePools?: Record<string, number>` — stores the current charge / use count per pool declared in `Feature.resourcePoolTemplates`. Travels with the item instance on trade/transfer. Documented in `ARCHITECTURE.md` section 5.7 lifecycle table. Applied to SRD rings: Ring of the Ram (50 finite charges), Ring of Three Wishes (3 rubies), Ring of Spell Turning (3/day), Ring of Djinni Calling (1/day), all four Elemental Command rings (multiple pools), Ring of Shooting Stars.
...
- [x] **1.6 ResourcePool reset conditions — full set (incremental + calendar):** Extended `ResourcePool.resetCondition` in `src/lib/types/pipeline.ts` to an **8-value union**: `"short_rest"`, `"long_rest"`, `"encounter"`, `"never"` (full resets); `"per_turn"`, `"per_round"` (incremental ticks); `"per_day"`, `"per_week"` (calendar resets). Added optional `rechargeAmount?: number | string` field (formula-capable) for the amount restored per tick (incremental modes only). Documented the full `resetCondition` reference table in `ARCHITECTURE.md` section 4.4 — including the distinction between `"per_day"` (dawn reset, no sleep required) and `"long_rest"` (requires 8 hours of sleep), and Fast Healing vs Regeneration design notes. Engine trigger methods `triggerDawnReset()` and `triggerWeeklyReset()` implemented in Phase 3.6.
...
- [x] **2.5c `attacker.*` modifier target namespace:** Added support for modifiers that impose penalties on the attacker. (a) Documented `"attacker."` prefix convention on `Modifier.targetId` in `src/lib/types/pipeline.ts` JSDoc — these modifiers never enter the owner's static pipeline totals, resolved only at roll time. (b) Added `resolveAttackerMods(defenderMods, targetTags, targetPipeline)` internal helper to `diceEngine.ts`: strips the prefix, matches pipeline, applies `situationalContext` filtering. (c) Added optional `defenderAttackerMods?: Modifier[]` 7th parameter to `parseAndRoll()`. (d) Added `RollResult.attackerPenaltiesApplied?: Modifier[]` transparency field (absent when no penalties apply). (e) Documented in `ARCHITECTURE.md` section 4.6. Applied to SRD: all four Elemental Command rings now carry `attacker.combatStats.attack_bonus` modifier (value −1, situationalContext matching the associated elemental type).
...
- [x] **3.6 GameEngine tick, rest & calendar action methods:** Added `triggerTurnTick()`, `triggerRoundTick()`, `triggerEncounterReset()`, `triggerShortRest()`, and `triggerLongRest()` public methods to `GameEngine.svelte.ts`. Added private helpers `#getEffectiveMax()` and `#applyIncrementalTick()`. Also added `triggerDawnReset()` — resets all `"per_day"` character-level pools AND calls `#resetItemPoolsByCondition("per_day")` for item instance pools (Phase 1.3e/1.4a integration); and `triggerWeeklyReset()` — same for `"per_week"`. All methods drive the `resetCondition` lifecycle from Phase 1.6. `rechargeAmount` formulas evaluated via the Math Parser. Required by: Fast Healing, Regeneration, Combat Tab "New Encounter" button (Phase 10.1), Long Rest button, X/day and X/week item ability tracking.
...
- [x] **3.8 GameEngine item mechanics & activation dispatch methods:** Added the following methods to `GameEngine.svelte.ts`. (a) `initItemResourcePools(instance, feature)` — idempotent: populates `instance.itemResourcePools` for any `resourcePoolTemplates` entry not yet present; never overwrites an existing (even zero) value. (b) `getItemPoolValue(instanceId, poolId)` — returns current charge count, falling back to `template.defaultCurrent` for uninitialised pools. (c) `spendItemPoolCharge(instanceId, poolId, amount?)` — atomically decrements, floored at 0. (d) `#resetItemPoolsByCondition(condition)` — private: iterates all active instances, resets matching item pools; called by `triggerDawnReset()` / `triggerWeeklyReset()`. (e) `activateWithTier(instanceId, tierIndex)` — validates tier, resolves cost, checks balance, deducts charges, returns transient `grantedModifiers` or `null` on failure. (f) `getReactionFeaturesByTrigger(triggerEvent)` — returns all active features with `actionType: "reaction"` and matching `triggerEvent`.
...
- [x] **17.8 Engine enhancement tests — resource pools, item charges, tiered activation, trigger dispatch, attacker modifiers:** Five new / extended test files covering the Phase 1.3e/1.4a/1.6/2.5c/3.6/3.8 additions.
    - _`resourcePool.test.ts` extended:_ Type-soundness for all 8 `resetCondition` values. `triggerDawnReset` resets only `"per_day"` (not `"long_rest"`, `"per_week"`, `"never"`). `triggerWeeklyReset` resets only `"per_week"`. Both are mutually independent. `triggerLongRest` unaffected by calendar pools. 12 new tests (2 type + 10 isolation).
    - _`itemResourcePools.test.ts` (new):_ 25 tests — `initItemResourcePools` idempotency (never resets an existing 0-valued pool), `spendItemPoolCharge` floor at 0, cross-instance independence (two Ring of Ram instances have separate pools), calendar reset with stashed (`isActive: false`) instances excluded.
    - _`tieredActivation.test.ts` (new):_ 16 tests — `activateWithTier` charge deduction per tier (0/1/2), tier-specific `grantedModifiers` returned, out-of-range guard → `null`, insufficient-charges guard → `null`, transient modifier isolation.
    - _`triggerActivation.test.ts` (new):_ 16 tests — `getReactionFeaturesByTrigger` returns matching `"reaction"` features, excludes `"passive"` (even with stray `triggerEvent`), excludes inactive instances.
    - _`attackerModifiers.test.ts` (new):_ 10 tests — air-elemental −1 penalty applied to `finalTotal`, situational context filtering, wrong-pipeline excluded, defender own pipeline `totalBonus` provably unchanged, `attackerPenaltiesApplied` absent (not empty array) when no match. 653 total tests pass.
...
_Pre-conversion engine gap analysis for D20SRD_CONVERSION.md task C-17 (Special Materials). Analysis of all 6 SRD special materials (Adamantine, Darkwood, Dragonhide, Cold Iron, Mithral, Alchemical Silver) revealed one genuine engine gap: the `combatStats.max_dex_bonus` pipeline was defined in C-08 armor item data (targeting modifiers) but NEVER initialized in GameEngine and therefore silently ignored. Mithral's "+2 maximum Dex bonus" effect REQUIRES this pipeline to be expressible as a dynamic `grantedModifier` rather than being hardcoded per-item. Additionally, the existing `setAbsolute` type used in C-08 data is incompatible with additive mithral bonuses — a new `"max_dex_cap"` modifier type with minimum-wins stacking was introduced._

_All other special material effects were confirmed to be fully expressible with existing primitives: DR grants via `type:"damage_reduction"`, ACP reductions via `combatStats.armor_check_penalty`, ASF reductions via `combatStats.arcane_spell_failure`, DR bypass tags (cold_iron, silver, adamantine), dragonhide non-metal tag, masterwork enhancement modifiers. The "one armor category lighter" movement effect for mithral is handled via a tag `"armor_movement_category_reduced"` and description (armor-speed penalty system is a future engine feature not yet implemented)._

- [x] **E-17a Type Extension + Engine Change:** Added `"max_dex_cap"` to `ModifierType` in `src/lib/types/primitives.ts` with full inline documentation (minimum-wins stacking, only meaningful on `combatStats.max_dex_bonus`, content authoring examples). Added `combatStats.max_dex_bonus` pipeline (`baseValue = 99` = uncapped) to GameEngine initial combatStats. Added Phase 3 special-case handling in `GameEngine.svelte.ts`: extracts `max_dex_cap` modifiers before `applyStackingRules`, takes minimum as `effectiveBaseValue`, applies remaining modifiers (e.g., mithral's untyped +2) normally, then `continue`s to skip general processing. Updated `07_d20srd_core_equipment_armor.json`: all 13 armor/shield items' `combatStats.max_dex_bonus` modifier type changed from `"setAbsolute"` → `"max_dex_cap"`. Updated `test_mock.json` similarly (3 items).

- [x] **E-17b Tests (`src/tests/maxDexBonus.test.ts`):** 22 new Vitest tests (863 total, all passing). 4 sections: (1) stacking rules treatment — verifies `max_dex_cap` is valid type, documents why Phase 3 intercepts before stackingRules; (2) Phase 3 computation via helper — no armor=99, single cap, mithral+cap=4, multi-cap min-wins, condition beats armor, identical caps, mithral on full plate; (3) content authoring contracts — armor mod structure, mithral mod structure, condition mod, shields without restriction (no modifier), tower shield type check; (4) edge cases — zero cap, mithral on zero cap, large cap, multiple untyped bonuses, pipeline independence.

- [x] **E-17c Documentation:** `ARCHITECTURE.md` section 4.17 (new subsection after §4.16) — full pipeline reference: two-layer model table (max_dex_cap minimum-wins + untyped additive), Phase 3 algorithm, 6-row example table, content authoring JSON examples, shields-without-restriction note, UI contract (ArmorClass panel, `effectiveDexToAC = min(dex, max_dex_bonus.totalValue)`). `CHECKPOINTS.md` updated with 8 new verification items.

**Total test count after Phase E-17: 863 tests (all passing).**
