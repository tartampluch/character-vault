...
- [ ] **Checkpoint #9 — User Management System** (requires Phase 22): Verify: admin bootstrap (empty DB seeds admin with empty password); no-password login + 7-day auto-suspend + `needs_password_setup` flag; `PUT /api/auth/setup-password`; `requireAdmin()` vs `requireGameMaster()` role boundary; all 7 user CRUD endpoints respect self-edit restrictions; campaign_users join table CRUD; SessionContext role/needsPasswordSetup fields; `/setup-password` route and layout redirect guard; user management page (admin-only guard, full table, disabled self-row actions); UserFormModal + ConfirmDeleteModal; campaign Members section (suspended user support); PHPUnit + Vitest coverage for all Phase 22 paths. Resolve all CRITICAL and MAJOR issues before proceeding.
